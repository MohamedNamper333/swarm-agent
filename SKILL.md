---
name: swarm-agent
description: "Use the Swarm Agent — a collaborative team of 9 workers (1 coordinator + 8 specialists) + 2 dedicated vision agents, with 679 specialised skills (plus 1680+ plugin skills on-demand) distributed across them — to analyse, design, build, review, debug, or visually inspect any type of project."
...
679 core skills → 9 domains → 9 workers + 2 vision agents → 1 coordinated swarm.
...
|------|------|--------|-------------|
| 🥇 Stable | Prime | 679 مهارة مثبتة ومختبرة | المهام الإنتاجية اليومية |
| 🥈 Verified | Plugin | 1680+ مهارة من FrancoStino vault | التوسع عند الحاجة |
...
```json
{
 ...
 "mode": "primary",
 "model": "opencode/big-pickle",
 "description": "Swarm: 9 workers + 2 vision agents + 679+ skills + auto-verdict",
 "color": "accent"
 },
 "innovator": {
 "mode": "subagent",
...
4. Spawns all 9 in parallel via `Task()` with `swarm-worker` agent
5. Collects results
...
| **Plugin** (FrancoStino) | **1680+** | On-demand via SkillPointer vault — no token overhead |

**Total: 679 core skills + 1680+ plugin skills = 2359+ skills available**
